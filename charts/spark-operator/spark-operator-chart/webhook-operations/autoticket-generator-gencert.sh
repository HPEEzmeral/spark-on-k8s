...
DNS.2 = autoticket-generator-svc.{CONFIGURE-NAMESPACE}
DNS.3 = autoticket-generator-svc.{CONFIGURE-NAMESPACE}.svc
EOF

update_replace_yaml() {
  data_bytes=`cat $CERT_FILENAME | base64 | tr -d '\n'`
  sed -i 's,{autoticket-csr-data},'$data_bytes',g' autoticket-generator-csr.yaml
  sed -i 's,{configure-namespace},'$NAMESPACE',g' autoticket-generator-csr.yaml
}
...
genservicecert() {
  ...
  echo "Adding namespace to $CONFIG_FILENAME file"
  sed -i 's,{CONFIGURE-NAMESPACE},'$NAMESPACE',g' $CONFIG_FILENAME
...
genk8csr() {
  ...
  kubectl certificate approve autoticket-generator-svc.$NAMESPACE

  # verify CSR has been created

  for (( i=0; i<3; ++i)); do

    encoded_server_cert=`kubectl get csr autoticket-generator-svc.$NAMESPACE -o jsonpath={.status.certificate}`
  ...
  sed -i 's,{configure-namepsace},'$NAMESPACE',g' autoticket-generator-mwhconfig.yaml
  sed -i 's,{autoticket-servercert-encoded},'$encoded_server_cert',g' autoticket-generator-vwhconfig.yaml
  sed -i 's,{configure-namepsace},'$NAMESPACE',g' autoticket-generator-vwhconfig.yaml

}

create_autoticket_generator_secret() {
  kubectl create secret generic autoticket-generator-certs -n $NAMESPACE --from-file=key.pem=$1 --from-file=cert.pem=$2
}
